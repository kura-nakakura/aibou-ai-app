...
/**
 * LpBuilder — LP / ホームページ作成（Claude のデザイン生成のような体験）.
 *
 *  1. 作りたいものを書く → 1ファイル完結のHTMLを生成
 *  2. 右側でライブプレビュー（PC/スマホ幅を切替）／コード表示
 *  3. 「もっと〇〇して」と指示して反復デザイン（既存HTMLを渡して直す）
 *  4. .html でダウンロード、または生成物として保存
 *
 * 生成HTMLは外部CDNに依存しない自己完結なので、iframe にそのまま流せる。
 */
...
export default function LpBuilder() {
  ...
  const run = async (refine: boolean) => {
    ...
    if (!text || busy) return;
    setBusy(true);
    setNote(refine ? "修正中…" : "デザインを生成中…（20〜40秒ほどかかります）");
    try {
      const r = await lpGenerate({ brief: text, style, current: refine ? html : "" });
      if (r.error || !r.html) {
  ...
  const download = () => {
    ...
    a.href = url;
    a.download = `${(title || "landing-page").replace(/[^\p{L}\p{N}_-]/gu, "_").slice(0, 40)}.html`;
  ...
  const save = async () => {
    setBusy(true);
    try {
      const r = await lpGenerate({ brief: title || "保存", style, current: html, save: true });
      setNote(r.artifact ? "✓ 生成物に保存しました" : `⚠ ${r.error ?? "保存に失敗"}`);
    } catch { setNote("⚠ 保存に失敗しました"); } finally { setBusy(false); }
  };

  if (!API_URL) {
    return <div className="panel p-3 text-[11px] leading-relaxed text-muted">LP作成はバックエンド接続後に使えます（DIAGNOSTICS参照）。</div>;
  }
      ...
      <div className="flex min-h-0 flex-col gap-2 overflow-y-auto">
        <div className="panel p-3">
          ...
          <textarea
            ...
            rows={5}
            placeholder={html
              ? "例：ヒーローの色を暖色に。料金表を3プランにして、FAQを増やして"
              : "例：カフェのLP。自家焙煎が強みで、店舗情報と予約ボタンを入れて"}
        ...
        {html && (
          <div className="panel p-3">
            ...
            <p className="mt-2 text-[9px] leading-relaxed text-muted">
              ※ 外部CDNに依存しない1ファイルHTMLです。ダウンロードしてそのまま公開できます。
            </p>
          </div>
        )}
      </div>
          ...
          {!html ? (
            <div className="grid h-full place-items-center p-6 text-center">
              <p className="text-[11px] leading-relaxed tracking-[0.14em] text-muted/60 label-mono">
                左に作りたいページを書いて「デザインを生成」<br />
                生成後は「もっと〇〇して」で何度でも直せます
              </p>
            </div>
          ) : view === "code" ? (
